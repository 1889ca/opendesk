...
function labeledInput(
  ...
  const span = el('span', 'wf-prop-label', labelText);
...
export function createPropertiesPanel(
  ...
  function renderEmpty() {
    container.innerHTML = '';
    const msg = el('p', 'wf-prop-empty', 'Select a node or edge to edit properties');
    container.appendChild(msg);
  }

  function renderNode(node: WorkflowNode) {
    container.innerHTML = '';
    const heading = el('h3', 'wf-prop-heading', `${node.type.replace('_', ' ')} node`);
    ...
    if (node.type === 'trigger') {
      renderTriggerConfig(container, node, callbacks);
    } else if (node.type === 'condition') {
      renderConditionConfig(container, node, callbacks);
    } else if (node.type === 'action') {
      renderActionConfig(container, node, callbacks);
    }
  ...
  function renderEdge(edge: WorkflowEdge) {
    container.innerHTML = '';
    const heading = el('h3', 'wf-prop-heading', 'Edge');
    container.appendChild(heading);
...
function renderTriggerConfig(
  ...
  const triggerType = (node.config.triggerType as string) ?? 'document.updated';
  container.appendChild(
    labeledSelect('Trigger type', [
      ...
      callbacks.onNodeUpdate(node.id, {
        config: { ...node.config, triggerType: val },
      });
    }),
  );
}
...
function renderConditionConfig(
  ...
  container.appendChild(
    labeledInput('Field', cfg.field ?? '', (val) => {
      callbacks.onNodeUpdate(node.id, {
        config: { ...node.config, field: val },
      });
    }),
  );

  container.appendChild(
    labeledSelect('Operator', [
      ...
      callbacks.onNodeUpdate(node.id, {
        config: { ...node.config, operator: val },
      });
    }),
  );

  container.appendChild(
    labeledInput('Value', cfg.value ?? '', (val) => {
      callbacks.onNodeUpdate(node.id, {
        config: { ...node.config, value: val },
      });
    }),
  );
}
...
function renderActionConfig(
  ...
  container.appendChild(
    labeledSelect('Action type', [
      ...
      callbacks.onNodeUpdate(node.id, {
        config: { ...node.config, actionType: val, actionConfig: {} },
      });
    }),
  );
  ...
  for (const f of fields) {
    container.appendChild(
      labeledInput(f.label, actionConfig[f.key] ?? '', (val) => {
        const updated = { ...actionConfig, [f.key]: val };
        callbacks.onNodeUpdate(node.id, {
          config: { ...node.config, actionConfig: updated },
        });
      }),
    );
  }
