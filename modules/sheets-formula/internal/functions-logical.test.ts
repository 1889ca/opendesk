...
describe('IS* predicates', () => {
  ...
  it('ISNA detects only #N/A errors', () => {
    const g = grid({ A1: 1 });
    expect(evaluateFormula('=ISNA(MATCH(99, A1:A1, 0))', g, 'B1')).toBe(true);
    expect(evaluateFormula('=ISNA(1/0)', grid({}), 'A1')).toBe(false);
  });
});
...
describe('TYPE', () => {
  it('returns 1 for numbers', () => {
    expect(evaluateFormula('=TYPE(1)', grid({}), 'A1')).toBe(1);
  });
  it('returns 2 for strings', () => {
    expect(evaluateFormula('=TYPE("hi")', grid({}), 'A1')).toBe(2);
  });
