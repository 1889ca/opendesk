...
export interface KbEntityStore {
  ...
  updateEntity(id: string, updates: EntityUpdates): Promise<EntityRow | null>;
  deleteEntity(id: string): Promise<boolean>;
  searchEntities(workspaceId: string, query: string, limit?: number): Promise<EntityRow[]>;
}

export function createKbEntityStore(pool: Pool): KbEntityStore {
  ...
  async function getEntity(id: string): Promise<EntityRow | null> {
    const result = await pool.query<EntityRow>(
  ...
  return { createEntity, getEntity, listEntities, updateEntity, deleteEntity, searchEntities };
}
