...
export interface KbDatasetStore {
  insertRows(entryId: string, rows: DatasetRowInput[], startIndex?: number): Promise<DatasetRow[]>;
  getRows(entryId: string, opts?: { limit?: number; offset?: number }): Promise<DatasetRow[]>;
  getRowCount(entryId: string): Promise<number>;
  updateRow(rowId: string, entryId: string, data: Record<string, unknown>): Promise<DatasetRow | null>;
  deleteRow(rowId: string, entryId: string): Promise<boolean>;
  clearRows(entryId: string): Promise<number>;
  replaceRows(entryId: string, rows: DatasetRowInput[]): Promise<DatasetRow[]>;
}

export function createKbDatasetStore(pool: Pool): KbDatasetStore {
  ...
  async function getRowCount(entryId: string): Promise<number> {
  ...
  async function deleteRow(rowId: string, entryId: string): Promise<boolean> {
  ...
  async function clearRows(entryId: string): Promise<number> {
    const result = await pool.query(
  ...
  return { insertRows, getRows, getRowCount, updateRow, deleteRow, clearRows, replaceRows };
}
